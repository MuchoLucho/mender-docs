...
### Pseudo Subsystems
These refer to enhanced configurations and logic extensions that build upon existing subsystems. They are designed to handle complex event detection scenarios, providing advanced customization while leveraging the underlying subsystems.
...
In the monitor library source, you will find the definition for the creation of the Check:
...
An example or usecase of this pseudo subsytem is to monitor for `kill` event on a container named `scanner` you need to create a check with
